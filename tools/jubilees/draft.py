#!/usr/bin/env python3
"""draft.py — produce an AI draft for one Jubilees chapter."""
...
TOOL_NAME = "submit_jubilees_draft"
TOOL_REASON_VALUES = {
    "alternative_reading",
    "lexical_alternative",
    "textual_variant",
...
You will submit your draft by calling the `submit_jubilees_draft` function exactly once. Do not output any other text — only the function call.

Translation philosophy: optimal equivalence (balanced formal/dynamic), but preserve apocalyptic-revelatory density and chronology language carefully.

Never:
- paraphrase beyond what the Ge'ez warrants
- flatten Jubilees into generic Genesis/Exodus paraphrase
- erase calendrical terminology
- omit significant lexical decisions just to make the chapter smoother
- copy existing English translations
"""
...
SUBMIT_TOOL = {
    ...
    "function": {
        "name": TOOL_NAME,
        "description": "Submit a draft English translation for one Jubilees chapter, including major lexical and theological decisions.",
        "parameters": {
            "type": "object",
            "required": [
                "english_text",
...
def call_azure_openai(*, system: str, user: str, model: str, temperature: float, max_completion_tokens: int = 16000) -> tuple[dict[str, Any], str]:
...
def build_record(bundle: build_chapter_prompt.ChapterPromptBundle, tool_input: dict[str, Any], *, model_id: str, model_version: str, prompt_id: str, prompt_sha256: str, temperature: float, output_hash: str) -> dict[str, Any]:
    record: dict[str, Any] = {
        "id": f"JUB.{bundle.chapter:03d}",
        "reference": f"Jubilees {bundle.chapter}",
        "unit": "chapter",
        "book": "Jubilees",
        "source": {
            ...
            "rows": bundle.source_payload["rows"],
            "validation_modes": bundle.source_payload["validation_modes"],
        },
        "translation": {
...
def draft_chapter(chapter_num: int, *, model: str = DEFAULT_MODEL_ID, temperature: float = DEFAULT_TEMPERATURE, prompt_id: str = "jubilees_chapter_draft_v1", write: bool = True) -> DraftResult:
    bundle = build_chapter_prompt.build_jubilees_chapter_prompt(chapter_num)
    prompt_sha = sha256_hex(SYSTEM_PROMPT + "\n\n---\n\n" + bundle.prompt)
    tool_input, model_version = call_azure_openai(system=SYSTEM_PROMPT, user=bundle.prompt, model=model, temperature=temperature)
    ...
    return DraftResult(chapter=chapter_num, record=record, output_path=output_path, prompt_sha256=prompt_sha, model_version=model_version)
...
def main() -> int:
    ...
    bundle = build_chapter_prompt.build_jubilees_chapter_prompt(args.chapter)
    if args.dry_run:
        print("=" * 72)
        print("SYSTEM")
        print("=" * 72)
