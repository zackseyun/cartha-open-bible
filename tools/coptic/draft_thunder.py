#!/usr/bin/env python3
"""draft_thunder.py — produce an AI draft for one Thunder, Perfect Mind line block."""
...
import build_thunder_lineblock_prompt as bp  # noqa: E402
...
You MUST follow the doctrinal stance and translation philosophy in the DOCTRINE.md and PHILOSOPHY.md excerpts provided. You will submit your draft by calling the `submit_thunder_lineblock_draft` function exactly once. Do not output any other text — only the function call.

Translation philosophy: optimal equivalence, but in poetry/paradox contexts preserve the sharpness and strangeness of the source over smoothing.

Never:
- Paraphrase from the English consult text.
- Smooth contradictory pairs into a single domesticated idea.
- Drop repeated “I am” structures just to avoid awkwardness.
- Pretend a low-confidence alignment is precise.
...
SUBMIT_TOOL = {
    ...
    'function': {
        'name': TOOL_NAME,
        'description': 'Submit a draft English translation for one Thunder line block, including paradox/voice/alignment notes.',
        'parameters': {
            'type': 'object',
            'required': ['english_text','translation_philosophy','lexical_decisions','parallelism_check','voice_check','alignment_risk_note','revision_risk_note'],
            'properties': {
                ...
                'parallelism_check': {'type': 'string'},
                'voice_check': {'type': 'string'},
...
def validate_tool_input(tool_input: dict[str, Any]) -> None:
    ...
    for field in ('parallelism_check','voice_check','alignment_risk_note','revision_risk_note'):
        if not str(tool_input.get(field,'') or '').strip():
            errors.append(f'{field} must be non-empty')
    if errors:
        raise ValueError('; '.join(errors))


def output_path_for_segment(segment_id: str) -> pathlib.Path:
    return TRANSLATION_ROOT / f'{segment_id}.yaml'
...
def build_record(bundle: bp.PromptBundle, tool_input: dict[str, Any], *, model_id: str, model_version: str, prompt_id: str, prompt_sha256: str, temperature: float, output_hash: str) -> dict[str, Any]:
    record = {
        'id': f'THUNDER.{bundle.segment_id}',
        'reference': f'Thunder, Perfect Mind — {bundle.label}',
        'unit': 'line_block',
        ...
        'parallelism_check': tool_input.get('parallelism_check', ''),
        'voice_check': tool_input.get('voice_check', ''),
        'alignment_risk_note': tool_input.get('alignment_risk_note', ''),
        'ai_draft': {
            'model_id': model_id,
        ...
        'revision_risk_note': tool_input.get('revision_risk_note', ''),
    }
...
def write_yaml(record: dict[str, Any], segment_id: str) -> pathlib.Path:
    out = output_path_for_segment(segment_id)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(yaml.safe_dump(record, sort_keys=False, allow_unicode=True, default_flow_style=False), encoding='utf-8')
    return out


def draft_segment(segment_id: str, *, model: str = DEFAULT_MODEL_ID, temperature: float = DEFAULT_TEMPERATURE, max_completion_tokens: int = DEFAULT_MAX_COMPLETION_TOKENS, request_timeout_seconds: int = DEFAULT_REQUEST_TIMEOUT_SECONDS, prompt_id: str = 'thunder_lineblock_draft_v1', write: bool = True) -> DraftResult:
    bundle = bp.build_thunder_lineblock_prompt(segment_id)
    ...
    out = output_path_for_segment(segment_id)
    if write:
        out = write_yaml(record, segment_id)
    return DraftResult(segment_id, record, out, prompt_sha, model_version)
...
def main() -> int:
    ...
    args = ap.parse_args()
    bundle = bp.build_thunder_lineblock_prompt(args.segment)
    if args.dry_run:
    ...
    try:
        result = draft_segment(args.segment, model=args.model, temperature=args.temperature, max_completion_tokens=args.max_completion_tokens, request_timeout_seconds=args.request_timeout_seconds)
