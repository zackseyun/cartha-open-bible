...
def call_openrouter(*, system: str, user: str, model: str, temperature: float) -> tuple[dict[str, Any], str, str]:
    ...
    return _call_openai_compatible(api_key=api_key, base_url=OPENROUTER_BASE_URL, extra_headers={'HTTP-Referer': 'https://cartha.com', "X-Title": "People\'s Open Bible"}, tools=[openrouter_submit_tool()], system=system, user=user, model=model, temperature=temperature)
