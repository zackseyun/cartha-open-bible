#!/usr/bin/env python3
"""draft_didache.py — produce an AI draft for one Didache chapter.

This is the first actual drafting tool for the Phase D Greek
extra-canonical track. It consumes `tools/build_didache_prompt.py` and
writes chapter-level YAML under:
...
TOOL_NAME = "submit_didache_draft"
TOOL_REASON_VALUES = {
    "alternative_reading",
    "lexical_alternative",
    "textual_variant",
...
You are drafting ONE DIDACHE CHAPTER. Your job is to produce the highest-quality chapter draft you can, while exposing the major lexical and theological decisions so the result is fully auditable.

You MUST follow the doctrinal stance and translation philosophy in the DOCTRINE.md and PHILOSOPHY.md excerpts provided.

You will submit your draft by calling the `submit_didache_draft` function exactly once. Do not output any other text — only the function call.
...
- Flatten early-Christian moral / liturgical language into generic modern prose.
...
SUBMIT_TOOL = {
    ...
    "function": {
        "name": TOOL_NAME,
        "description": "Submit a draft English translation for one Didache chapter, including major lexical and theological decisions.",
        "parameters": {
            "type": "object",
            "required": [
                "english_text",
...
def build_record(
    bundle: build_didache_prompt.PromptBundle,
    ...
    *,
    ...
    record: dict[str, Any] = {
        "id": f"DID.{chapter.chapter:02d}",
        "reference": f"Didache {chapter.chapter}",
        "unit": "chapter",
        "book": "Didache",
        "source": {
...
def draft_chapter(
    ...
    *,
    ...
    prompt_id: str = "didache_chapter_draft_v1",
    write: bool = True,
) -> DraftResult:
    bundle = build_didache_prompt.build_didache_prompt(chapter_num)
...
def main() -> int:
    ...
    bundle = build_didache_prompt.build_didache_prompt(args.chapter)
    if args.dry_run:
        print("=" * 72)
        print("SYSTEM")
        print("=" * 72)
