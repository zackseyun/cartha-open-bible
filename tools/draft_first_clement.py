#!/usr/bin/env python3
"""draft_first_clement.py — produce an AI draft for one 1 Clement chapter."""
...
import build_first_clement_prompt


REPO_ROOT = pathlib.Path(__file__).resolve().parent.parent
TRANSLATION_ROOT = REPO_ROOT / "translation" / "extra_canonical" / "1_clement"
...
TOOL_NAME = "submit_first_clement_draft"
TOOL_REASON_VALUES = {
    "alternative_reading",
    "lexical_alternative",
    "textual_variant",
...
You are drafting ONE CHAPTER OF 1 CLEMENT. Your job is to produce the highest-quality chapter draft you can, while exposing the major lexical and theological decisions so the result is fully auditable.

You MUST follow the doctrinal stance and translation philosophy in the DOCTRINE.md and PHILOSOPHY.md excerpts provided.

You will submit your draft by calling the `submit_first_clement_draft` function exactly once. Do not output any other text — only the function call.
...
- Flatten the paraenetic, rhetorical, or ecclesial texture into generic modern prose.
...
SUBMIT_TOOL = {
    ...
    "function": {
        "name": TOOL_NAME,
        "description": "Submit a draft English translation for one 1 Clement chapter, including major lexical and theological decisions.",
        "parameters": {
            "type": "object",
            "required": [
                "english_text",
...
def build_record(
    bundle: build_first_clement_prompt.PromptBundle,
    ...
    *,
    ...
    record: dict[str, Any] = {
        "id": f"1CLEM.{chapter.chapter:02d}",
        "reference": f"1 Clement {chapter.chapter}",
        "unit": "chapter",
        "book": "1 Clement",
        "source": {
...
def draft_chapter(
    ...
    *,
    ...
    prompt_id: str = "first_clement_chapter_draft_v1",
    write: bool = True,
) -> DraftResult:
    bundle = build_first_clement_prompt.build_first_clement_prompt(chapter_num)
...
def main() -> int:
    ...
    bundle = build_first_clement_prompt.build_first_clement_prompt(args.chapter)
    if args.dry_run:
        print("=" * 72)
        print("SYSTEM")
        print("=" * 72)
